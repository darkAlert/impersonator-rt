...
class TestDensePose(object):
    def __init__(self, args):
        ...
        self._test(dst_dir='/home/darkalert/KazendiJob/Data/HoloVideo/Data/test/uv/t1')

    def _test(self, dst_dir):
        self._model.set_eval()
        for i_batch, test_batch in enumerate(self._dataset_test):
            # Inference:
            self._model.set_input(test_batch)
            pred_uvs, masks = self._model.inference()

            # GPU -> CPU:
            pred_uvs = pred_uvs.detach().cpu().numpy()
            masks = masks.detach().cpu().numpy()
            bs = pred_uvs.shape[0]

            # Filter uvs by mask:
            uvs = np.zeros((bs,256,256,3), dtype=np.uint8)
            for j in range(1, 25):
                b, x, y = np.where(masks[:, :, :] == j)
                uvs[b,x,y,1] = (pred_uvs[b, j-1, x, y, 0]*255).astype(np.uint8)
                uvs[b,x,y,2] = (pred_uvs[b, j-1, x, y, 1]*255).astype(np.uint8)
            uvs[:, :, :, 0] = masks[:, :, :]

            # Save:
            dst_subdir = os.path.join(dst_dir, str(i_batch))
            if not os.path.exists(dst_subdir):
                os.makedirs(dst_subdir)
            for i in range(bs):
                dst_path = os.path.join(dst_subdir, str(i).zfill(5) + '.png')
                cv2.imwrite(dst_path, uvs[i])

            # Break:
            if i_batch*self._opt.batch_size >= self._dataset_test_size:
                break
